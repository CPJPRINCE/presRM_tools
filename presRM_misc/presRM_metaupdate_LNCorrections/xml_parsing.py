...
def date_formatting(vardate):
    vardate = str(vardate)
    if vardate == "False" or vardate == "NaN" or vardate == "nan" or vardate == "NaT" or vardate == "" or vardate == "None": vardate = datetime.now().strftime("%Y-%m-%dT%H:%M:%S.000Z")
...
class XML_Parse():
    ...
    def metadata_parse(self,box_dict,item_dict=None,transfer_dict=None,type_flag=None,up_flag=None):
        ...
        if type_flag == "Box":
            ...
            rmsclassification = text_formatting(box_dict.get('RMS Classification Code'))
        ...
        xml_parse = f'<rm:rm xmlns:rm="{self.NS}" xmlns="{self.NS}">'\
                    ...
                    f"<rm:classificationcode>{rmsclassification}</rm:classificationcode>" \
                    "</rm:recordinfo>" \
                    "<rm:clientinfo>" \
